...
- Is there a use-trained armor proficiency, and if so how does it interact across three classes? (Weapon skills resolved in D-007/D-014: primary-only, per weapon group, use-trained with a competent floor and mastery as a proc/damage edge. Resources resolved in D-017; base attributes fold into D-015 functional buckets.)
