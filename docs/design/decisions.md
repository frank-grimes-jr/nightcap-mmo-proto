...
- **Every bucket has a solo-facing expression.** Each role bucket must have a self-directed form, not only a group-directed one, so role-based statistics never force a group identity and D-002 solo-capable content holds. Sustain covers self-heal, regeneration, and lifesteal as well as outward group healing; control covers locking down whatever is attacking the player, not only group crowd control; utility covers personal movement and resource economy. A rounded solo character spreads across buckets; a self-sufficient damage build leans offense plus self-sustain.
