...
- **Defense skill.** A single, character-wide use-trained defense skill improves how effectively a character *executes* active defensive actions (block, guard, parry) — their reliability, timing windows, and uptime. It trains the *act* of defending, not the numbers: how much a block mitigates and how often a hit is avoided remain governed by D-015 defense-bucket statistics (mitigation, avoidance), so no defensive effect is double-sourced by both a skill and a stat.
- Because the defense skill trains from all combat and has no weapon-group-style resets, it is self-maintaining — it tracks the character's level and rarely leaves a normally-playing character underdefended. It is one skill, not per-class or per-tier, so it does not multiply across the three slots; the armor tier it operates within is still primary-set (D-014). The competent-floor principle applies: a character defends competently at a level-appropriate floor, with the skill as a mastery edge on execution.
- Exact rates, failure floors, and bad-luck protection remain unresolved. The exact block/parry/guard maneuvers the defense skill governs, and whether weapon mastery also unlocks higher weapon-group technique tiers, remain unresolved.
...
- **Universal eligibility:** universally-eligible items exist only as accessories. There are no universal armor pieces or weapons, so the chassis-tier and per-type weapon gates are never bypassed. Because universal items are accessories, their power is already bounded by the per-accessory cross-role ceiling above; universality adds discovery flavor, not a new power axis. Their rarity remains unresolved (tuning), but their power ceiling is settled by the accessory rule.

## D-015: Functional statistic vocabulary

- **Status:** Provisional
- **Date:** 2026-08-04
- Defines the functional statistic vocabulary referenced by D-013 (stats describe function, not class) and priced against the fixed item power budget.
- **Role-based buckets.** Statistics are organized into functional buckets by combat role — provisionally offense, defense, control, sustain, and utility (original names unresolved). There is no separate classic-attribute layer. What traditional attributes did collapses into the function they powered: spell power becomes offense, health regeneration becomes sustain, mana and resource economy become utility, and so on. Players allocate toward effects they feel directly, not toward an abstract attribute resolved through hidden derivation.
- **Non-class-coded test.** A statistic is legitimate when the effect it grants is wanted by multiple classes ("melee-efficiency stat"). It is an illegitimate class backdoor when it reads as one class's stat ("the rogue stat"). This is the same test D-013 applies to items, now applied to the vocabulary itself.
- **Every bucket has a solo-facing expression.** Each role bucket must have a self-directed form, not only a group-directed one, so role-based statistics never force a group identity and D-002 solo-capable content holds. Sustain covers self-heal, health regeneration, and lifesteal as well as outward group healing; control covers locking down whatever is attacking the player, not only group crowd control; utility covers personal movement and resource economy — the size and regeneration of the mana and Energy pools (D-017). Health recovery lives in sustain; mana and Energy pool economy lives in utility. A rounded solo character spreads across buckets; a self-sufficient damage build leans offense plus self-sustain.
